...
void SFMT::period_certification(void) {
    ...
    for (i = 0; i < 4; i++)
	inner ^= m_psfmt32[idxof(i)] & m_parity[i];
    for (i = 16; i > 0; i >>= 1)
	inner ^= inner >> i;
    inner &= 1;
    /* check OK */
 ...
 */
const char* SFMT::get_idstring(void) {
    return IDSTR;
}
 ...
 */
int SFMT::get_min_array_size32(void) {
    return N32;
}
 ...
 */
int SFMT::get_min_array_size64(void) {
    return N64;
}
...
void SFMT::init_gen_rand(uint32_t seed) {
    ...
    m_psfmt32[idxof(0)] = seed;
    for (i = 1; i < N32; i++) {
	m_psfmt32[idxof(i)] = 1812433253UL * (m_psfmt32[idxof(i - 1)] 
					    ^ (m_psfmt32[idxof(i - 1)] >> 30))
	    + i;
    }
    m_idx = N32;
 ...
 */
void SFMT::init_by_array(uint32_t *init_key, int key_length) {
    int i, j, count;
    uint32_t r;
    int lag;
    int mid;
    ...
    r = func1(m_psfmt32[idxof(0)] ^ m_psfmt32[idxof(mid)] 
	      ^ m_psfmt32[idxof(N32 - 1)]);
	...
	r = func1(m_psfmt32[idxof(i)] ^ m_psfmt32[idxof((i + mid) % N32)] 
    ...
    for (j = 0; j < N32; j++) {
	r = func2(m_psfmt32[idxof(i)] + m_psfmt32[idxof((i + mid) % N32)] 
		  + m_psfmt32[idxof((i + N32 - 1) % N32)]);
	m_psfmt32[idxof((i + mid) % N32)] ^= r;
	r -= i;
	m_psfmt32[idxof((i + mid + lag) % N32)] ^= r;
