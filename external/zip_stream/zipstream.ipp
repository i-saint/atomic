...
namespace detail{
		...
		do
		{
			...
			if (m_err == Z_OK  || m_err == Z_STREAM_END)
			{
				...
				if ( (remainder = written_byte_size%sizeof(char_type))!=0)
				{
					...
					memcpy(
						&(m_output_buffer[0]), 
						&(m_output_buffer[(size_t)written_byte_size-remainder]),
						remainder);
		...
		>::fill_input_buffer()
	{
		...
		return m_zip_stream.avail_in=(size_t)m_istream.gcount()*sizeof(char_type);
	}
			...
			)
	{
		...
		m_crc = crc32( 
			...
			(size_t)buffer_size_ - m_zip_stream.avail_out/sizeof(char_type)
			);	
